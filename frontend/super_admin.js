...
async function loadTrainersDetailed() {
    try {
        const res = await fetch('/api/super/trainers-detailed', {
            headers: { 'Authorization': `Bearer ${masterToken}` }
        });
...
window.openTrainerModal = async (trainer = null) => {
    ...
    try {
        const gRes = await fetch('/api/super/gyms', { headers: { 'Authorization': `Bearer ${masterToken}` } });
        if (gRes.ok) {
...
async function loadGyms() {
    const res = await fetch('/api/super/gyms', { headers: { 'Authorization': `Bearer ${masterToken}` } });
...
async function loadAllStudents() {
    const res = await fetch('/api/super/students-all', { headers: { 'Authorization': `Bearer ${masterToken}` } });
...
async function loadMarketplaceData() {
    const res = await fetch('/api/super/marketplace-data', { headers: { 'Authorization': `Bearer ${masterToken}` } });
...
async function loadBillingData() {
    const res = await fetch('/api/super/billing-data', { headers: { 'Authorization': `Bearer ${masterToken}` } });
    const data = await res.json();
...
async function loadCatalogMgmt() {
    const res = await fetch('/api/super/catalog-mgmt', { headers: { 'Authorization': `Bearer ${masterToken}` } });
...
async function loadExercises() {
    try {
        ...
        `).join('') || '<tr><td colspan="4">Nenhum exercício cadastrado.</td></tr>';
    } catch (e) { console.error(e); }
}
